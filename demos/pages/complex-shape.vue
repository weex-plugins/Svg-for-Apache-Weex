<template>
  <div class="page">
    <navbar back="true" title="Advanced Demos"></navbar>
    <scroller class="main">
      <text class="desc">Some Svg Figures</text>
      <div class="item">
        <svg style="width:363px; height:177px;" viewBox="0 0 121 59" ><path d="M61.072 32.55c.227 0 .455.147.487.38l.714 6.92c.032.32.13.554.26.554s.26-.233.292-.554l.78-6.92c.064-.233.162-.38.39-.38h3.31c.163 0 .358.147.39.322l.844 6.978c.033.32.13.554.26.554s.26-.233.293-.554l.65-6.95c.03-.203.162-.35.357-.35h5.26c.325 0 .422.176.39.41l-3.02 21.05c-.033.292-.228.438-.422.438h-4.74a.4.4 0 0 1-.39-.32l-1.235-7.067c-.033-.26-.13-.466-.26-.466s-.227.234-.26.497L64.287 54.1c-.065.204-.163.35-.357.35h-4.807a.467.467 0 0 1-.454-.38L55.55 32.93c-.033-.263.13-.38.357-.38h5.162zM77.21 32.93c0-.175.195-.38.422-.38H89.16c.194 0 .324.146.324.322v5.255c0 .175-.162.292-.357.292h-5.423c-.13 0-.227.087-.227.203v1.636c0 .115.13.203.227.203h5.48c.13 0 .227.088.227.204v5.314a.27.27 0 0 1-.26.264h-5.543c-.13 0-.228.088-.228.234V48.2c0 .117.097.205.194.205h5.618c.227 0 .358.146.358.292v5.43c0 .176-.163.32-.358.32H77.567c-.195 0-.357-.174-.357-.378V32.93zM91.66 32.93c0-.175.194-.38.422-.38h11.527c.194 0 .324.146.324.322v5.255c0 .175-.163.292-.357.292h-5.423c-.13 0-.227.087-.227.203v1.636c0 .115.13.203.227.203h5.48c.13 0 .227.088.227.204v5.314a.27.27 0 0 1-.26.264h-5.544c-.13 0-.227.088-.227.234V48.2c0 .117.097.205.194.205h5.618c.227 0 .357.146.357.292v5.43c0 .176-.164.32-.358.32H92.016c-.194 0-.357-.174-.357-.378V32.93zM109.194 43.47a.63.63 0 0 0 .065-.233c0-.06-.033-.205-.066-.292l-3.702-10.073c-.065-.146 0-.322.195-.322h5.942c.13 0 .227.117.26.234l.746 3.037c.033.117.097.205.13.205.065 0 .13-.088.162-.204l.747-3.036c.033-.146.13-.234.26-.234h5.942c.13 0 .26.088.26.234 0 .058 0 .117-.032.204l-3.702 9.84c-.065.117-.098.32-.098.38 0 .058.033.233.065.32l3.767 10.628c.065.205 0 .292-.195.292h-6.04c-.097 0-.194-.087-.227-.204l-.747-3.59c-.032-.118-.097-.176-.13-.176-.065 0-.097.058-.13.175l-.746 3.59c-.033.118-.163.205-.293.205h-5.976c-.13 0-.194-.058-.194-.146 0-.058 0-.088.032-.146l3.702-10.686z" stroke="#F5F5F5" stroke-width=".9" fill="#515151"/><path d="M34.93 21.862a2.734 2.734 0 0 1 0-5.466 2.734 2.734 0 0 1 0 5.466m20.622-10.31a20.59 20.59 0 0 0-5.766-4.86C34.99-4.104 14.29-1.036 3.256 13.506 7.303 8.28 13.39 5.503 19.57 5.472 8.03 10.708 0 22.312 0 35.8c0 3.77.837 7.627 1.993 11.005 1.66-1.508 3.173-5.484 4.948-6.862.87 4.27 2.554 10.537 4.886 14.043 2.168-4.383 3.93-7.445 7.157-11.055.88 6.3 4.505 11.052 8.398 15.71a33.137 33.137 0 0 1 3.138-15.576c.156.076.313.15.472.224A33.468 33.468 0 0 0 41.56 55.13c-1.486-3.093-2.32-6.452-2.788-9.984a33.586 33.586 0 0 1-.294-4.416c0-3.223-.1-6.338.754-9.286.4-1.374.51-2.71 1.075-4.006.22-.503.45-1 .695-1.49 1.054-2.12 2.147-4.147 3.747-6.126.303-.376.802-.745 1.12-1.108 1.435-1.412 4.15-2.285 6.324-2.285 4.123 0 7.406 2.918 7.8 6.95-.146-4.694-1.658-8.434-4.442-11.828" fill="#00B4FF" /><path d="M35.975 17.44a1.69 1.69 0 1 1 .002 3.38 1.69 1.69 0 0 1-.002-3.38" fill="#515151"/></svg>
        <text class="desc">weex logo</text>
      </div>
      <div class="item">
        <svg style="width:300px;height:300px"
           viewBox="0 0 300 300">
          <path fill="#1ba1e2" d="M155.3,39.7c26.9,1.1,51.2,11.9,69.7,29l0,0c-18.5,20.2-30.2,46.8-31.4,76.1h-38.3C155.3,144.7,155.3,39.7,155.3,39.7z
           M145.7,39.7v105.1h-38.3c-1.2-29.3-12.8-55.9-31.4-76.1l0,0C94.5,51.6,118.9,40.8,145.7,39.7z M155.3,259.3V154.3h38.3
          c1.2,29.3,12.8,55.9,31.4,76.1l0,0C206.5,247.4,182.1,258.2,155.3,259.3z M145.7,259.3c-26.9-1.1-51.2-11.9-69.7-29
          c18.5-20.2,30.2-46.8,31.4-76.1h38.3C145.7,154.3,145.7,259.3,145.7,259.3z M231.8,75.4c16.8,18.4,27.4,42.6,28.6,69.3l0,0h-57.2
          C204.3,118.1,214.9,93.9,231.8,75.4L231.8,75.4L231.8,75.4L231.8,75.4z M231.8,223.6L231.8,223.6L231.8,223.6
          c-16.8-18.4-27.4-42.6-28.6-69.3h57.2C259.2,180.9,248.6,205.1,231.8,223.6L231.8,223.6z M40.7,144.7c1.1-26.6,11.8-50.8,28.6-69.3
          l0,0c16.8,18.4,27.4,42.6,28.6,69.3H40.7L40.7,144.7z M40.7,154.3h57.2c-1.1,26.6-11.8,50.8-28.6,69.3
          C52.4,205.1,41.8,180.9,40.7,154.3L40.7,154.3z M150.5,269c66,0,119.5-53.5,119.5-119.5S216.5,30,150.5,30S31,83.5,31,149.5
          S84.5,269,150.5,269L150.5,269z"/>
        </svg>
        <svg style="width:120px; height:120px;" viewBox="0 0 32 32">
          <path fill="#333333" d="M8 8.5v-5.5h16l-5.5 5.5 5.5 5.5h-15v15h-1v-20.5zM9 4v9h12.5l-4.5-4.5 4.5-4.5h-12.5z"></path>
        </svg>
        <svg style="width:120px; height:120px;" viewBox="0 0 32 32">
          <path fill="#2ecc71" d="M20.638 20h-8.332l2.244 1.98 2.267 0.011-1.697 0.009h3.378l0.020-0.018-0.099 0.001 2.218-1.983zM21.757 19l3.243-2.9v-8.091c0-0.557-0.455-1.009-1-1.009h-15c-0.552 0-1 0.444-1 1.002v8.198l3.173 2.8h10.583zM13.091 6l3.409-3 3.409 3h4.093c1.107 0 1.997 0.895 1.997 2v3.36l3 2.64v14.006c0 1.1-0.897 1.994-2.004 1.994h-20.993c-1.114 0-2.004-0.893-2.004-1.994v-14.006l3-2.64v-3.36c0-1.112 0.894-2 1.997-2h4.093zM18.428 6l-1.928-1.7-1.928 1.7h3.856zM26 12.677v2.656l1.5-1.333-1.5-1.323zM7 15.333v-2.656l-1.5 1.323 1.5 1.333zM13.5 23l-7 6h20l-7-6h-6zM27.685 28.725v0 0c0.194-0.183 0.315-0.442 0.315-0.725v-13l-8 7.017 7.685 6.708zM5.315 28.725l7.685-6.708-8-7.017v13c0 0.283 0.121 0.542 0.315 0.725v0 0zM10 10v1h13v-1h-13zM10 13v1h13v-1h-13zM10 16v1h13v-1h-13z"></path>
        </svg>
        <svg style="width:120px; height:120px;" viewBox="0 0 32 32">
          <path fill="#f1c40f" d="M16 23l9 6-4-10 9-6h-10l-4-10-4 10h-10l9 6-4 10 9-6zM16 21.753l-6.8 4.547 3.2-7.7-7.2-4.6h7.5l3.3-8.5 3.3 8.5h7.5l-7.2 4.6 3.2 7.7-6.8-4.547z"></path>
        </svg>
        <text class="desc">Some Svg Icons</text>
      </div>
      <div class="item">
        <svg style="width:400px;height:300px;">
          <defs>
            <linearGradient id="trunkgrad" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stop-color="#996d2e" stop-opacity="1" />
              <stop offset="100%" stop-color="#d5bc79" stop-opacity="1" />
            </linearGradient>
            <radialGradient id="shadow" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
              <stop offset="0%" stop-color="#666666"/>
              <stop offset="100%" stop-color="#cccccc" />
            </radialGradient>
          </defs>
          <ellipse cx="188" cy="273" rx="80" ry="20" fill="url(#shadow)" />
          <ellipse cx="188" cy="273" rx="24" ry="5" fill="url(#shadow)" />
          <polygon points="182,211 187,211 187,274 180,274" fill="url(#trunkgrad)" />
          <polygon points="187,211 196,211 198,274 187,274" fill="#996d2e" />

          <polygon points="187,28 154,162 116,159" fill="#a5df66" stroke="#a5df66" />
          <polygon points="154,162 116,159 155,218" fill="#79b544" stroke="#79b544" />

          <polygon points="187,28 216,114 154,162"  fill="#619b37" stroke="#619b37" />
          <polygon points="187,28 216,114 264,162 204,28" fill="#619b37" stroke="#619b37" />
          <polygon points="216,114 264,162 223,210" fill="#54892f" stroke="#54892f" />
          <polygon points="216,114 223,210 154,162" fill="#5c9433" />

          <polygon points="154,162 223,210 188,211" fill="#54892f" stroke="#54892f" />
          <polygon points="154,162 188,211 155,218" fill="#54892f" stroke="#54892f" />

          <polygon points="200,70 216,114 185,137" fill="#79b544" stroke="#79b544" />
          <polygon points="216,114 220,167 185,137" fill="#588f32" />
          <polygon points="216,114 250,148 220,167" fill="#598d33" />

        </svg>
        <text class="desc">A tree by  Luke Reid</text>
      </div>
      
    </scroller>
    
  </div>
   
</template>

<style>
  .page{
    flex: 1;
    padding-top: 20px;
    background-color: #fff;
  }
 
  .main{
    padding-top: 88px;
  }
  .desc{
    margin: 20px;
    font-size: 28px;
    text-align: left;
    color: #444;
  }
  .item{
    align-items: center;
    margin: 20px;
    padding: 10px;
    border-bottom: 2px solid #ddd;
  }
  .item-shape{
    width: 600px;
    height: 480px;
  }
</style>

<script>
  import navbar from '../include/navbar.vue';
  module.exports = {
    components: {
      navbar  
    },
    
    
  };
  
  
</script>